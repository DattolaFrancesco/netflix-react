...
class MyCarousel extends Component {
  ...
  getSaga = () => {
    const link = `http://www.omdbapi.com/?apikey=4315c223&s=${this.props.saga}`;
    fetch(link)
      .then((res) => {
        if (res.ok) {
          return res.json();
  ...
  render() {
    const filmsFirst = this.state.films.slice(0, 5);
    const filmsSecond = this.state.films.slice(5);
    let arrows = this.state.loaded;
      ...
      <Container className="text-center">
        {!this.state.loaded && (
          <Carousel interval={null} indicators={false} controls={false}>
            <Carousel.Item>
              <Row className="g-1">{this.Placeholder()}</Row>
            </Carousel.Item>
          </Carousel>
        )}
        {!this.state.toast && (
          <Toast className="position-absolute bottom-0 end-0">
            ...
            <Toast.Body className="bg-dark text-danger fw-bold fs-3">{this.state.error}</Toast.Body>
          </Toast>
        )}
